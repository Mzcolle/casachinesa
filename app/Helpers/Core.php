...
class Core
{
    ...
    public static function DiscountBalance(Wallet $wallet, $bet)
    {
        ...
        // Verifica se o saldo comum é suficiente
        if ($wallet->balance >= $bet) {
            $wallet->decrement('balance', $bet);
            return 'balance';
        }
        // Verifica se o saldo para saque é suficiente
        elseif ($wallet->balance_withdrawal >= $bet) {
            $wallet->decrement('balance_withdrawal', $bet);
            return 'balance_withdrawal';
        }
        // Verifica se a soma dos saldos comuns e para saque é suficiente
        ...
        // Nova regra: Verifica se a soma de todos os saldos é suficiente
        elseif ($wallet->balance + $wallet->balance_withdrawal + $wallet->balance_bonus >= $bet) {
            $remainingBet = $bet;

            // Debita do saldo comum
            if ($wallet->balance > 0) {
                if ($wallet->balance >= $remainingBet) {
                    $wallet->decrement('balance', $remainingBet);
                    return 'balance';
                } else {
                    $remainingBet -= $wallet->balance;
                    $wallet->decrement('balance', $wallet->balance);
                }
            }

            // Debita do saldo para saque
            if ($wallet->balance_withdrawal > 0) {
                if ($wallet->balance_withdrawal >= $remainingBet) {
                    $wallet->decrement('balance_withdrawal', $remainingBet);
                    return 'balance_withdrawal';
                } else {
                    $remainingBet -= $wallet->balance_withdrawal;
                    $wallet->decrement('balance_withdrawal', $wallet->balance_withdrawal);
                }
            }

            // Debita do saldo de bônus
            if ($wallet->balance_bonus > 0) {
                if ($wallet->balance_bonus >= $remainingBet) {
                    $wallet->decrement('balance_bonus', $remainingBet);
                    return 'balance_bonus';
                }
            }
        }

        // Se nenhum saldo for suficiente
    ...
    public static function initializeMissions($userId)
    {
        // Fetch all available missions
        $missions = MissionDeposit::all();

        foreach ($missions as $mission) {
            // Check if the user already has this mission registered in MissionDepositUser
            $existingEntry = MissionDepositUser::where('user_id', $userId)
                ->where('mission_deposit_id', $mission->id)
                ->first();

            if (!$existingEntry) {
                // Create the entry with status 0 (incomplete)
                MissionDepositUser::create([
                    'user_id' => $userId,
                    'mission_deposit_id' => $mission->id,
                    'status' => 0, // Initial status as incomplete
                    'wallet_bonus' => $mission->bonus_amount, // Set wallet_bonus to bonus_amount from MissionDeposit
                ]);
            }
        }
    }
    ...
    {
        $caracteresPermitidos = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'; // Apenas números
...
//
   ...
   // }
    ...
    /**
     * @dev 
     * @param $key agent_secret_key
     * @param $message 
     * @return string
     */
    public static function getSign($key, $message)
    {
     ...
     * @return string
     */
    public static function codificarUserID($userID)
    {
        $base_convert = base_convert($userID, 10, 36);
        // Garantir que a string codificada tenha no mínimo 4 caracteres e no máximo 48 caracteres
        return str_pad($base_convert, 4, "0", STR_PAD_LEFT);
    }

    /**
     * @dev 
     * @param $userIdCodificado ID do usuário codificado
     * @return string
     */
    public static function decodificarUserID($userIdCodificado)
    {
